      ...
      if (ip6->ip6_nxt == IPPROTO_UDP) {
        ...
        if (ntohs(udp->dest) == 69) {
          ...
          if (opcode == 0x0001 || opcode == 0x0002) {
            ...
            char *p = ((char *)tftp) + 2;
            char *q = file_name;
            mempcpy(file_name, ((char *)tftp) + 2, strlen(((char *)tftp) + 2));
            char mode[] = "octet";
            ...
            new_transfer.client_tid = ntohs(udp->source);
            ...
            if (opcode == 1) {
              ...
              if (new_transfer.fp) {
                ...
                reply_tftp[tftp_len++] = 0x00;
                reply_tftp[tftp_len++] = 0x05;

                reply_tftp[tftp_len++] = 0x00;
                reply_tftp[tftp_len++] = 0x01;

                memcpy(reply_tftp + 4, "File not found", 14);
                tftp_len += 14;
                reply_tftp[tftp_len++] = 0x00;
              ...
              if (new_transfer.fp) {
                ...
                reply_tftp[tftp_len++] = 0x00;
                reply_tftp[tftp_len++] = 0x06;

                memcpy(reply_tftp + 4, "File already exists", 19);
                tftp_len += 19;
                reply_tftp[tftp_len++] = 0x00;
                ...
                uint16_t udp_len = 4 + sizeof(udphdr);
                uint16_t ip_len = udp_len + sizeof(ip6_hdr);
                reply_udp->uh_ulen = htons(udp_len);
                reply_ip6->ip6_plen = htons(udp_len);
                validateAndFillChecksum(output, ip_len);
          ...
          for (int i = 0; i < transfers.size(); i++) {
            ...
            if (current_transfer.client_tid == ntohs(udp->source) &&
                current_transfer.server_tid == ntohs(udp->dest) &&
                current_transfer.client_addr == ip6->ip6_src) {
              if (current_transfer.is_read) {
                ...
                if (opcode == 0x0004) {
                  ...
                  if (block_number == current_transfer.last_block_number) {
                    ...
                    if (current_transfer.state == InTransfer) {
                      ...
                      uint8_t buffer[512];
                      size_t block_size =
                          fread(buffer, 1, sizeof(buffer), current_transfer.fp);
                      ...
                      udphdr *reply_udp = (udphdr *)&output[sizeof(ip6_hdr)];
                      reply_udp->uh_sport = htons(current_transfer.server_tid);
                      reply_udp->uh_dport = htons(current_transfer.client_tid);

                      uint8_t *reply_tftp =
                          (uint8_t *)&output[sizeof(ip6_hdr) + sizeof(udphdr)];
                      reply_tftp[0] = 0x00;
                      reply_tftp[1] = 0x03;
                      ((tftp_hdr *)reply_tftp)->block_number =
                          htons(++current_transfer.last_block_number);
                      memcpy(reply_tftp + 4, buffer, block_size);

                      uint16_t udp_len = 4 + block_size + sizeof(udphdr);
                      uint16_t ip_len = udp_len + sizeof(ip6_hdr);
                      reply_udp->uh_ulen = htons(udp_len);
                      reply_ip6->ip6_plen = htons(udp_len);
                      validateAndFillChecksum(output, ip_len);
                      ...
                      memcpy(current_transfer.last_block_data, buffer,
                             block_size);
                      current_transfer.last_block_size = block_size;
                    ...
                    ((tftp_hdr *)reply_tftp)->block_number =
                        htons(current_transfer.last_block_number);
                    memcpy(reply_tftp + 4, current_transfer.last_block_data,
                           current_transfer.last_block_size);

                    uint16_t udp_len =
                        4 + current_transfer.last_block_size + sizeof(udphdr);
                    uint16_t ip_len = udp_len + sizeof(ip6_hdr);
                    reply_udp->uh_ulen = htons(udp_len);
                    reply_ip6->ip6_plen = htons(udp_len);
                    validateAndFillChecksum(output, ip_len);
                ...
                if (opcode == 0x03) {
                  ...
                  if (block_number == current_transfer.last_block_number + 1) {
                    ...
                    uint16_t block_size = ntohs(udp->len) - sizeof(udphdr) - 4;
                    fwrite(((uint8_t *)tftp) + 4, 1, block_size,
                           current_transfer.fp);
