  ...
  while (!done) {
      ...
      if (HAL_GetNeighborMacAddress(dest_if, nexthop, &dest_mac) == 0) {
        ...
        if (HAL_GetTicks() - last_time > 1000) {
          ...
          for (int i = 0; i < strlen(argv[3]); i++) {
            reply_tftp[tftp_len++] = argv[3][i];
          }
          reply_tftp[tftp_len++] = 0x00;
          // TODO（4 行）
          // 传输模式字段，设为 octet
          for (int i = 0; i < 5; i++) {
            reply_tftp[tftp_len++] = "octet"[i];
          }
          reply_tftp[tftp_len++] = 0x00;
    ...
    if (dst_is_me) {
      ...
      if (ip6->ip6_nxt == IPPROTO_UDP) {
        ...
        if (ntohs(udp->dest) == current_transfer.client_tid) {
          ...
          if (current_transfer.server_tid == 0) {
            ...
            if (ntohs(udp->source) != current_transfer.server_tid) {
              continue;
            }
          }
          ...
          if (opcode == 0x03) {
            ...
            if (block_number == current_transfer.last_block_number + 1) {
              ...
              fwrite(((uint8_t *)tftp) + 4, 1, block_size, current_transfer.fp);
            ...
            udphdr *reply_udp = (udphdr *)&output[sizeof(ip6_hdr)];
            reply_udp->uh_sport = htons(current_transfer.client_tid);
            reply_udp->uh_dport = htons(current_transfer.server_tid);
            ...
            if (block_number == current_transfer.last_block_number) {
              ...
              if (current_transfer.state == InTransfer) {
                ...
                uint8_t buffer[512];
                size_t block_size = fread(buffer, 1, 512, current_transfer.fp);
                ...
                validateAndFillChecksum(output, ip_len);
                HAL_SendIPPacket(if_index, output, ip_len, src_mac);
                memcpy(current_transfer.last_block_data, buffer, block_size);
